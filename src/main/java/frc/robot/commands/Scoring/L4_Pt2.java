    ...
    addCommands(   
      Commands.parallel( //do in parallel so elevator stays up the whole time
        ...
        // new WaitCommand(1.2), //wait for elevator to go up
          ...
          new CoralRelease(coralHold, Constants.CoralHold.L4_RELEASE_SPEED).withTimeout(0.2),//0.5
          new PIDCoralPivot(coralPivot, Constants.CoralPivot.ENC_REVS_FULL_RETRACT).withTimeout(0.7)//do not change timeout
      )
      ),
